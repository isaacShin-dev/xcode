...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]){
        ...
        CLGeocoder().reverseGeocodeLocation(pLocation!, completionHandler: {(placemark, Error) -> Void in let pm = placemark!.first
    ...
    @IBAction func sgChangeLocation(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0{
            ...
            locationManager.startUpdatingLocation()
            setAnnotation(latitudeValue: 37.55924, longitudeValue: 127.02163, delta: 0.01, title: "우리집", subtitle: "서울 특별시 중구 다산로 36길 109")
